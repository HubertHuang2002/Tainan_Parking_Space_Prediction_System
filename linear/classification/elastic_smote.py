...
def load_and_preprocess_data(file_paths):
    ...
    # check if column ParkingSegmentID exist; skip specific data handling if missing
    if 'ParkingSegmentID' in combined_df.columns and 'half_hour_interval' in combined_df.columns:
    ...
    # if both avg_available_spots and TotalSpaces exist, perform data cleaning
    if 'avg_available_spots' in combined_df.columns and 'TotalSpaces' in combined_df.columns:
    ...
    if 'TotalSpaces' in combined_df.columns:
...
def prepare_features(train_df, valid_df, test_df):
    ...
    selected_features = [
        ...
        'weekday_num_0', 'pressure_msl', 'vapour_pressure_deficit', 'month_val_11', 
    ...
    for df in all_dfs:
        ...
        # keep only the target variable and specified features
        available_features = []
        ...
        # keep the target variable and available features
        cols_to_keep = ['is_full'] + available_features
        cols_to_keep = [col for col in cols_to_keep if col in df_processed.columns]
    ...
    for col in processed_dfs[0].columns:
        if col == 'is_full':  # target variable
            continue
        if processed_dfs[0][col].dtype == 'object' or col == 'district':
    ...
    for df in processed_dfs:
        if len(df) > 0:
            for col in df.columns:
                if col != 'is_full' and df[col].dtype in ['float64', 'int64']:
    ...
    if len(X_sets[0]) > 0:
        print(f"Selected feature count: {X_sets[0].shape[1]}")
        print(f"Selected features: {X_sets[0].columns.tolist()}")
        
        # check which specified features are actually used
        used_features = set(X_sets[0].columns.tolist())
        requested_features = set(selected_features)
        missing_features = requested_features - used_features
        if missing_features:
...
def main():
    # load training data
    file_paths = [
        '/kaggle/input/dataset3/smote.csv'
    ]
    ...
    # check if the is_full column exists; if not, create it
    if 'is_full' not in df.columns:
        print("Creating target variable...")
        df = create_target_variable(df)
    else:
        print("Target variable 'is_full' already exists in data")
        print(f"Target distribution:")
        print(f"  Not full (0): {(df['is_full'] == 0).sum()} ({(df['is_full'] == 0).mean():.2%})")
        print(f"  Full (1): {(df['is_full'] == 1).sum()} ({(df['is_full'] == 1).mean():.2%})")
    
    # check if feature addition is needed; skip if the data has already been processed
    if 'day_off' not in df.columns or 'half_hour_cos' not in df.columns:
        print("Adding features...")
        df = add_features(df)
    else:
        print("Features already exist in data, skipping feature engineering")
    
    # since smote.csv may not have a date column, modify the data splitting method
    # use train/validation/test split ratios 
    ...
    valid_df = df_shuffled[n_train:n_train+n_valid].copy()
    test_df = df_shuffled[n_train+n_valid:].copy()
    ...
    if external_test_df is not None:
        ...
        # check whether the external test data needs processing
        if 'is_full' not in external_test_df.columns:
            external_test_df = create_target_variable(external_test_df)
        if 'day_off' not in external_test_df.columns or 'half_hour_cos' not in external_test_df.columns:
